...
import { searchHotelOffers, HotelSearchParams, HotelOfferResponse } from "@/lib/travelApi";
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
        ...
        <div>
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className={`w-full justify-start text-left font-normal ${!checkInDate && "text-muted-foreground"}`}>
        ...
        <div>
          ...
          <Select value={roomQuantity.toString()} onValueChange={(value) => setRoomQuantity(parseInt(value))}>
            <SelectTrigger>
      ...
      {hotelOffers.length > 0 && (
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {hotelOffers.map((hotelOffer) => (
            <div key={hotelOffer.hotel.hotelId} className="border rounded-lg shadow-lg p-4">
      ...
      {hotelOffers.length === 0 && !isLoading && <p className="mt-4">No hotel offers found.</p>}
    </div>
  );
};
